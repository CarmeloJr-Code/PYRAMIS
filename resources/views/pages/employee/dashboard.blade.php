    ...
    <div class="grid gap-4 md:grid-cols-3">
        ...
        @can('access-expenses')
            <flux:card>
                ...
                <flux:text class="mt-2">{{ __('Record what the business spends and see where it goes.') }}</flux:text>
                <flux:button :href="route('employee.expenses.index')" variant="ghost" size="sm" class="mt-4" wire:navigate>
                    {{ __('Open') }}
                </flux:button>
            </flux:card>
        @endcan
        ...
        @can('access-restocking')
            <flux:card>
                <flux:heading size="lg">{{ __('Restocking') }}</flux:heading>
                <flux:text class="mt-2">{{ __('Send finished goods from the main branch out to the outlets.') }}</flux:text>
                <flux:button :href="route('employee.restocks.index')" variant="ghost" size="sm" class="mt-4" wire:navigate>
                    {{ __('Open') }}
                </flux:button>
            </flux:card>
        @endcan
